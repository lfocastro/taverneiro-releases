# Catálogo de Cartas / Componentes — Paper Dungeons

> Documento gerado a partir das fotos enviadas. Jogo identificado: **Paper Dungeons** (roll & write).
>
> **Aviso de leitura:** as fotos são de celular e alguns números/ícones são pequenos.
> Os campos marcados com `(?)` são leitura aproximada e devem ser conferidos com a carta física.
>
> Convenção dos atributos das cartas de monstro:
> - **Os três números na base** (azul / laranja / vermelho) representam três níveis/tiers de dificuldade.
> - **Badge `×2`**: a carta possui o indicador de multiplicador/duplicidade no canto superior direito.

---

## Imagem 1 — Cartas de Monstro (frente) — 12 cartas

Dispostas em 4 colunas × 3 linhas.

### Linha 1 (borda verde)

#### Carta 01 — Minotauro
- Borda: verde
- Nível azul: **9**
- Nível laranja: **13**
- Nível vermelho: **17**
- Badge ×2: não

#### Carta 02 — Gárgula
- Borda: verde
- Nível azul: **9**
- Nível laranja: **13**
- Nível vermelho: **17**
- Badge ×2: não

#### Carta 03 — Golem
- Borda: verde
- Nível azul: **9**
- Nível laranja: **13**
- Nível vermelho: **17**
- Badge ×2: **sim**

#### Carta 04 — Múmia
- Borda: verde
- Nível azul: **9**
- Nível laranja: **13**
- Nível vermelho: **17**
- Badge ×2: não

### Linha 2 (borda roxa / laranja)

#### Carta 05 — Vampiro
- Borda: roxa
- Nível azul: **14**
- Nível laranja: **18**
- Nível vermelho: **22**
- Badge ×2: não

#### Carta 06 — Quimera
- Borda: laranja
- Nível azul: **14**
- Nível laranja: **18**
- Nível vermelho: **22**
- Badge ×2: não

#### Carta 07 — Elemental
- Borda: laranja
- Nível azul: **14**
- Nível laranja: **18**
- Nível vermelho: **22**
- Badge ×2: não

#### Carta 08 — Lich
- Borda: roxa
- Nível azul: **18**
- Nível laranja: **23**
- Nível vermelho: **28**
- Badge ×2: **sim**

### Linha 3 (borda vermelha / azul-petróleo)

#### Carta 09 — Dragão
- Borda: vermelha
- Nível azul: **18**
- Nível laranja: **23**
- Nível vermelho: **28**
- Badge ×2: não

#### Carta 10 — Observador
- Borda: roxa
- Nível azul: **18**
- Nível laranja: **23**
- Nível vermelho: **28**
- Badge ×2: não

#### Carta 11 — Hidra
- Borda: azul-petróleo (teal)
- Nível azul: **18**
- Nível laranja: **23**
- Nível vermelho: **28**
- Badge ×2: **sim**

#### Carta 12 — Gigante
- Borda: azul-petróleo (teal)
- Nível azul: **14**
- Nível laranja: **18**
- Nível vermelho: **22**
- Badge ×2: **sim**

#### Tabela resumo — Imagem 1

| # | Monstro | Borda | Azul | Laranja | Vermelho | ×2 |
|---|---------|-------|------|---------|----------|----|
| 01 | Minotauro | verde | 9 | 13 | 17 | — |
| 02 | Gárgula | verde | 9 | 13 | 17 | — |
| 03 | Golem | verde | 9 | 13 | 17 | ✔ |
| 04 | Múmia | verde | 9 | 13 | 17 | — |
| 05 | Vampiro | roxa | 14 | 18 | 22 | — |
| 06 | Quimera | laranja | 14 | 18 | 22 | — |
| 07 | Elemental | laranja | 14 | 18 | 22 | — |
| 08 | Lich | roxa | 18 | 23 | 28 | ✔ |
| 09 | Dragão | vermelha | 18 | 23 | 28 | — |
| 10 | Observador | roxa | 18 | 23 | 28 | — |
| 11 | Hidra | teal | 18 | 23 | 28 | ✔ |
| 12 | Gigante | teal | 14 | 18 | 22 | ✔ |

---

## Imagem 2 — Cartas de Grade/Masmorra (12 cartas)

Doze cartas com uma grade quadrada (estilo masmorra) e alguns marcadores
coloridos (amarelo / verde / vermelho / azul) posicionados em células
específicas — provavelmente o layout de surgimento/movimento de cada monstro.

> **Observação:** as grades não estão rotuladas com o nome do monstro na foto,
> então não é possível mapear com certeza cada grade ao seu monstro correspondente
> apenas pela imagem. Cada carta abaixo está identificada pela posição na foto
> (4 colunas × 3 linhas). Para mapear grade → monstro, é necessária uma foto
> mais próxima ou a informação do verso.

| Posição | Linha | Coluna | Marcadores visíveis (aprox.) |
|---------|-------|--------|------------------------------|
| Grade 01 | 1 | 1 | tokens amarelo/verde (?) |
| Grade 02 | 1 | 2 | tokens amarelo/verde (?) |
| Grade 03 | 1 | 3 | token amarelo (?) |
| Grade 04 | 1 | 4 | token amarelo (?) |
| Grade 05 | 2 | 1 | token verde (?) |
| Grade 06 | 2 | 2 | tokens verde/amarelo (?) |
| Grade 07 | 2 | 3 | tokens verde/amarelo (?) |
| Grade 08 | 2 | 4 | token amarelo (?) |
| Grade 09 | 3 | 1 | tokens verde/vermelho (?) |
| Grade 10 | 3 | 2 | tokens verde/amarelo (?) |
| Grade 11 | 3 | 3 | tokens vermelho/amarelo (?) |
| Grade 12 | 3 | 4 | tokens vermelho/amarelo (?) |

---

## Imagem 3 — Tabuleiro do Jogador (folha de masmorra)

Folha principal do jogador de **Paper Dungeons** ("PAPER DUNGEONS" impresso no topo).

Elementos identificados:

- **Mapa da masmorra:** grade de **6 colunas × 7 linhas**
  - Colunas rotuladas **A, B, C, D, E, F**
  - Linhas rotuladas **1 a 7**
  - Várias células já preenchidas com ícones (portas, escadas, itens, monstros, paredes)
- **Trilha de XP / Nível:** trilha numérica lateral esquerda, de **1 a 24**
- **Trilhas de heróis / recursos:** colunas de marcação no lado esquerdo
  (ícones de espada, escudo, poções, etc.)
- **Trilha de pergaminhos (scrolls):** faixa na parte inferior
- **Trilha de poções (potions):** faixa na parte inferior com numeração
  (valores crescentes: 5, 10, 15, 20, 25, 30, 36, 42, 48 — aprox.)
- **Faixa numérica inferior:** sequência **1 … 25** (pontuação/recursos)

> Os valores exatos das trilhas devem ser conferidos com uma foto mais próxima.

---

## Imagem 4 — Cartas de Objetivo (visão geral) — 16 cartas

Conjunto de 16 cartas de objetivo, dispostas em 4 colunas × 4 linhas.
Todas seguem o formato:

> **RECEBA `2`, `4` OU `6` SE VOCÊ … (condição com três limiares).**

Cada carta concede **2 / 4 / 6 pontos** conforme o jogador atinge o
1º, 2º ou 3º limiar da condição. No rodapé há ícones de recursos.

> **Observação:** nesta foto de conjunto a maioria dos textos está pequena
> e ilegível. As 4 cartas lidas com confiança estão na **Imagem 5** abaixo.
> As demais 12 cartas ficam pendentes de foto mais nítida.

---

## Imagem 5 — Cartas de Objetivo (close-up) — 4 cartas legíveis

#### Objetivo A — Itens Mágicos
- Recompensa: **2 / 4 / 6**
- Condição: **SE VOCÊ COMPLETOU 2, 4 OU 7+ ITENS MÁGICOS**
- Limiares: 2 itens → 2 pts · 4 itens → 4 pts · 7+ itens → 6 pts

#### Objetivo B — Salas dos Cantos
- Recompensa: **2 / 4 / 6**
- Condição: **SE VOCÊ EXPLOROU 1, 2 OU 3+ SALAS DOS CANTOS**
- Limiares: 1 sala → 2 pts · 2 salas → 4 pts · 3+ salas → 6 pts

#### Objetivo C — Heróis no Nível 5
- Recompensa: **2 / 4 / 6**
- Condição: **SE SEU GRUPO POSSUIR 2, 3 OU 4 HERÓIS NO NÍVEL 5**
- Limiares: 2 heróis → 2 pts · 3 heróis → 4 pts · 4 heróis → 6 pts

#### Objetivo D — Salas nas Bordas
- Recompensa: **2 / 4 / 6**
- Condição: **SE VOCÊ EXPLOROU 10, 13 OU 16+ SALAS NAS BORDAS**
- Limiares: 10 salas → 2 pts · 13 salas → 4 pts · 16+ salas → 6 pts

---

## Imagem 6 — Cartas "1º Jogador A…" (visão geral) — 16 cartas

Conjunto de 16 cartas de feito/conquista, dispostas em 4 colunas × 4 linhas.
Todas seguem o formato:

> **1º JOGADOR A … (cumprir determinado feito).**

Cada carta possui uma fita/medalha e vale **4 pontos** (badge `4`) para o
**primeiro jogador** que cumprir a condição.

Leituras parciais (foto de conjunto, texto pequeno — `(?)` = aproximado):

| Posição | Linha/Col | Condição (parcial) |
|---------|-----------|--------------------|
| 01 | 1/1 | 1º a DERROTAR a … (?) |
| 02 | 1/2 | 1º a COLETAR 4 … (?) |
| 03 | 1/3 | 1º a ALCANÇAR nível 12 (?) |
| 04 | 1/4 | 1º a EVOLUIR … (?) |
| 05 | 2/1 | 1º a ATRAVESSAR … (?) |
| 06 | 2/2 | 1º a ALCANÇAR 8 … (?) |
| 07 | 2/3 | 1º a COMPLETAR … item mágico (?) |
| 08 | 2/4 | 1º a DERROTAR … (?) |
| 09 | 3/1 | 1º a EVOLUIR … (?) |
| 10 | 3/2 | 1º a EXPLORAR … salas (?) |
| 11 | 3/3 | 1º a EVOLUIR TODOS … (?) |
| 12 | 3/4 | 1º a EVOLUIR … (?) |
| 13 | 4/1 | 1º a EXPLORAR TODAS … (?) |
| 14 | 4/2 | 1º a EXPLORAR TODAS … salas (?) |
| 15 | 4/3 | 1º a EXPLORAR … salas (?) |
| 16 | 4/4 | 1º a EXPLORAR … (?) |

> **Observação:** todas as condições acima precisam de fotos mais próximas
> para leitura confiável (como foi feito na Imagem 5).

---

## Pendências / Para refinar

- Confirmar dígitos exatos das cartas de monstro (foto mais nítida por carta).
- Mapear cada **grade da Imagem 2** ao **monstro correspondente** da Imagem 1.
- Capturar os ícones extras no canto superior das cartas de monstro
  (ataques/habilidades especiais), que não estão legíveis na foto atual.
- Detalhar os valores exatos das trilhas do tabuleiro (Imagem 3).
- Ler as **12 cartas de objetivo restantes** (Imagem 4) com fotos mais próximas.
- Ler as **16 cartas "1º Jogador A…"** (Imagem 6) com fotos mais próximas.
